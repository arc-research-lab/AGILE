...
max_itr=1
save_path=./results/spmv/${nvme_blk_size}
...
do  
    ...
    do 
        ...
        # sleep 5
        echo "./scripts/run.sh bin/pr -it ${max_itr} -nn ${info[4]} -en ${info[7]} -i ${i}/pr-rows.bin -gsn ${cache_page_size} -wo $((${max_ssd_page} * ${nvme_blk_size} * 512 / 4))  -qn ${queue_num} -qd ${queue_depth} -bd 128 -ss $((${nvme_blk_size} * 512)) -sbn $((${max_ssd_page} * 2)) | tee log"
        ./scripts/run.sh bin/pr -it ${max_itr} -nn ${info[4]} -en ${info[7]} -i ${i}/pr-rows.bin -gsn ${cache_page_size} -wo $((${max_ssd_page} * ${nvme_blk_size} * 512 / 4))  -qn ${queue_num} -qd ${queue_depth} -bd 128 -ss $((${nvme_blk_size} * 512)) -sbn $((${max_ssd_page} * 2)) | tee log
