...
__global__ void concate_kernel(float * d_bottom_out, float * d_vector_mean, float * d_all_features, \
    unsigned int batch_size, unsigned int table_size, unsigned int embedding_dim)
{

    unsigned int b = blockIdx.x; // batch
    unsigned int v_idx = threadIdx.x; // embedding vector element index

    // copy bottom out to d_vector_mean
    unsigned int src = b * embedding_dim + v_idx;
    unsigned int dst = b * embedding_dim * (table_size + 1) + v_idx;
    d_all_features[dst] = d_bottom_out[src];
    
    // copy embedding vector mean to d_all_features
    for(unsigned int i = 0; i < table_size; ++i){
        src = b * embedding_dim * table_size + i * embedding_dim + v_idx;
        dst = b * embedding_dim * (table_size + 1) + (i + 1) * embedding_dim + v_idx;
        d_all_features[dst] = d_vector_mean[src];
    }
    ...
    unsigned int batch_size, unsigned int table_size, unsigned int embedding_dim)
{
    ...
    concate_kernel<<<batch_size, embedding_dim, 0, stream>>>(d_bottom_out, d_vector_mean, d_all_features, batch_size, table_size, embedding_dim);
