...
struct Settings
{
    ...
    uint32_t batch_size;
    uint32_t dense_input_dim;
    uint32_t sparse_input_dim;
    uint32_t table_size;
    uint32_t iteration;
    uint32_t table_offset;
    uint32_t enable_compute;
    uint32_t enable_load;
...
{
    OptionMap parsers = {
        // {'i', OptionPtr(new Option<const char*>(input, "path", "input", "File path where the vertex file is. Provide .bel path."))},
        // {'o', OptionPtr(new Option<const char*>(output, "path", "output", "Result file path."))},
        // {'n', OptionPtr(new Option<uint32_t>(node_num, "number", "node_num", "How many nodes in the graph"))},
        // {'e', OptionPtr(new Option<uint64_t>(edge_num, "number", "edge_num", "How many edges in the graph"))},
...
//        {'n', OptionPtr(new Range(numReqs, 1, (uint64_t)std::numeric_limits<uint64_t>::max, "reqs", "number of reqs per thread", "1"))},
       {'p', OptionPtr(new Range(numPages, 1, (uint64_t)std::numeric_limits<uint64_t>::max, "pages", "number of pages in cache", "65536 * 8"))},
        {'a', OptionPtr(new Range(pageSize, 1, (uint64_t)std::numeric_limits<uint64_t>::max, "page_size", "size of page in cache", "4096"))},
        {'d', OptionPtr(new Range(queueDepth, 2, 65536, "queue_depth", "queue depth per queue", "256"))},
        {'q', OptionPtr(new Range(numQueues, 1, 65536, "num_queues", "number of queues per controller", "128"))},
...
//        {'e', OptionPtr(new Range(numElems, 1, (uint64_t)std::numeric_limits<uint64_t>::max, "num_elems", "number of 64-bit elements in backing array", "2147483648"))},
        ...
        {'E', OptionPtr(new Option<uint32_t>(embedding_dim, "number", "embedding_dim", "embedding vector dim", "64"))},
        {'B', OptionPtr(new Option<uint32_t>(batch_size, "number", "batch_size", "batch_size", "128"))},
        {'D', OptionPtr(new Option<uint32_t>(dense_input_dim, "number", "dense_input_dim", "dense_input_dim", "512"))},
        {'S', OptionPtr(new Option<uint32_t>(sparse_input_dim, "number", "sparse_input_dim", "sparse_input_dim", "64"))},
        {'T', OptionPtr(new Option<uint32_t>(table_size, "number", "table_size", "table_size", "8"))},
        {'I', OptionPtr(new Option<uint32_t>(iteration, "number", "iteration", "iteration", "100"))},
        {'O', OptionPtr(new Option<uint32_t>(table_offset, "number", "table_offset", "table_offset", "1024"))},
        {'C', OptionPtr(new Option<uint32_t>(enable_compute, "number", "enable_compute", "enable_compute", "1"))},
        {'L', OptionPtr(new Option<uint32_t>(enable_load, "number", "enable_load", "enable_load", "1"))},
    };
