...
template <>
void Option<float>::parseArgument(const char* optstr, const char* optarg)
{
    // char* endptr = nullptr;
...
void Settings::parseArguments(int argc, char** argv)
{
    OptionMap parsers = {
        ...
        {'w', OptionPtr(new Option<uint32_t>(weight_offset, "number", "weight_offset", "weight offsets in the ssd", "0"))},
        {'s', OptionPtr(new Option<uint32_t>(total_ssd_pages, "number", "total_ssd_pages", "Total number of SSD pages used", "2097152"))},
        {'m', OptionPtr(new Option<uint32_t>(max_itr, "number", "max_itr", "max iteration in page rank", "100"))},
        {'e', OptionPtr(new Option<float>(error_thresh, "number", "error_thresh", "the threshold for PageRank to stop", "0"))},
        {'n', OptionPtr(new Option<uint32_t>(node_num, "number", "node_num", "How many nodes in the graph", "0"))},
        {'n', OptionPtr(new Option<uint64_t>(edge_num, "number", "edge_num", "How many edges in the graph", "0"))},
