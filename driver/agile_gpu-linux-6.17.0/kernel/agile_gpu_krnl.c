...
static long ioctl_pin_gpu_buffer(struct file *file, unsigned int cmd, unsigned long arg);
static long ioctl_unpin_gpu_buffer(struct file *file, unsigned int cmd, unsigned long arg);
static long ioctl_allocate_dram_buffer(struct file *file, unsigned int cmd, unsigned long arg);
static long ioctl_free_dram_buffer(struct file *file, unsigned int cmd, unsigned long arg);
...
static int fp_mmap(struct file *filp, struct vm_area_struct *vma) {
    ...
    if(vars->mmap_flag == MMAP_GPU){
        ...
        vm_flags_reset_once(vma, vma->vm_flags | VM_DONTEXPAND | VM_DONTDUMP);
        pfn = params->phy_addr >> PAGE_SHIFT;
        pr_info("mmaping phy mem addr=0x%llx size=%zu at user virt addr=0x%lx\n", 
             params->phy_addr, size, vma->vm_start);
...
long ioctl_allocate_dram_buffer(struct file *file, unsigned int cmd, unsigned long arg){
    ...
    buf->vaddr_krnl = (void *)__get_free_pages(GFP_KERNEL | GFP_DMA, get_order(buf->size));
    if (!buf->vaddr_krnl)
        return -ENOMEM;
    {
        int i;
        struct page *pg = virt_to_page(buf->vaddr_krnl);
        for (i = 1; i < (1 << get_order(buf->size)); i++)
            init_page_count(pg + i);
    }
    ...
    if (copy_to_user((struct cpu_dram_buf *)arg, buf, sizeof(*buf))) {
        int i, np = 1 << get_order(buf->size);
        struct page *pg = virt_to_page(buf->vaddr_krnl);
        for (i = 0; i < np; i++) __free_page(pg + i);
        buf->vaddr_krnl = NULL;
        buf->phy_addr = 0;
        buf->size = 0;
        return -EFAULT;
