...
int ctrl_chrdev_create(struct ctrl* ctrl, dev_t first, const struct file_operations* fops)
{
    ...
    /* Mark as created (use a non-NULL sentinel since we no longer have a device struct) */
    ctrl->chrdev = (struct device*) 1UL;

    printk(KERN_INFO "Character device /dev/%s created (%d.%d) - use: mknod /dev/%s c %d %d\n",
