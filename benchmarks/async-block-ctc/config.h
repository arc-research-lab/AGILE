...
public:
    ...
    // Application config
    unsigned int buf_per_blk;
    unsigned int compute_sim;
    unsigned int compute_itr;
    ...
    Configs(int argc, char ** argv){
        ...
        items.push_back(new ConfigItem<unsigned int>("slot_size", "-ss", 512, "Slot size of the cache", slot_size));
        ...
        // items.push_back(new ConfigItem<std::string>("nvme_bar", "-bar", "0x95400000", "PCIe bar address of target ssd", nvme_bar));
        // items.push_back(new ConfigItem<unsigned int>("bar_size", "-bar_size", 16384, "PCIe bar size of target ssd", bar_size));

        items.push_back(new ConfigItem<unsigned int>("queue_num", "-qn", 2, "Number of NVME queue pairs", queue_num));
        ...
        // parallelism config
        items.push_back(new ConfigItem<unsigned int>("block_dim", "-bd", 1, "Block dimension", block_dim));
        items.push_back(new ConfigItem<unsigned int>("thread_dim", "-td", 16, "Thread dimension", thread_dim));
        ...
        items.push_back(new ConfigItem<unsigned int>("buf_per_blk", "-bpblk", 1, "buf per block", buf_per_blk));
        items.push_back(new ConfigItem<unsigned int>("compute_sim", "-comp", 65536, "compute_sim", compute_sim));
        items.push_back(new ConfigItem<unsigned int>("compute_itr", "-citr", 1, "compute repeat", compute_itr));
        items.push_back(new ConfigItem<unsigned int>("iteration", "-itr", 1, "total repeat", iteration));
