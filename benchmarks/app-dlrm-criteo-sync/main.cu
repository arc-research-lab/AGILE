...
__global__ void testKernel(unsigned int * d_input_sparse){
    unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int val = d_input_sparse[idx];
    printf("d_input_sparse[%d]: %u\n", idx, val);
}
...
int main(int argc, char ** argv){
    ...
    for(int itr = 0; itr < cfg.iteration; ++itr){
        ...
        // load embedding
        mean_embedding<128>(enable_load, stream_compute, ctrl, d_sparse_in, d_vector_mean, 0, cfg.batch_size, cfg.sparse_input_dim, cfg.embedding_vec_dim);
