...
int main(int argc, char ** argv){
    ...
    std::chrono::high_resolution_clock::time_point start0, end0, s0, e0, s1, e1, s2, e2;
    double time0 = 0;
    double time1 = 0;
    double time2 = 0;
    ...
    for(int itr = 0; itr < cfg.iteration + 1; ++itr){
        ...
        prefetch_embedding<512>(enable_load && itr < cfg.iteration, stream_compute, ctrl, d_sparse_in0, 0, cfg.batch_size, cfg.sparse_input_dim, cfg.embedding_vec_dim);
        cuda_err_chk(cudaStreamSynchronize(stream_compute));
        e0 = std::chrono::high_resolution_clock::now();
        time0 += std::chrono::duration_cast<std::chrono::microseconds>(e0 - s0).count();
        ...
        time1 += std::chrono::duration_cast<std::chrono::microseconds>(e1 - s1).count();
        ...
        time2 += std::chrono::duration_cast<std::chrono::microseconds>(e2 - s2).count();
    ...
    std::cout << "Prefetch time: " << time0 / 1000000.0 << " s\n";
    std::cout << "Load time: " << time1 / 1000000.0 << " s\n";
    std::cout << "Compute time: " << time2 / 1000000.0 << " s\n";
