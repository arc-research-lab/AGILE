    ...
    unsigned int batch_size, unsigned int sparse_input_dim, unsigned int embedding_vector_dim)
{
    ...
    unsigned int batch_idx = tid / sparse_input_dim;
    unsigned int batch_offset = tid % sparse_input_dim;
    if(batch_idx >= batch_size){
        return;
    }

    unsigned long idx = d_sparse_ptr[batch_idx * sparse_input_dim + batch_offset] * embedding_vector_dim;
    ...
    unsigned int batch_size, unsigned int sparse_input_dim, unsigned int embedding_vector_dim)
{
    ...
    unsigned int total_threads = batch_size * sparse_input_dim;
    ...
    unsigned int batch_size, unsigned int sparse_input_dim, unsigned int embedding_vector_dim)
{
    ...
    float * line = d_vector_mean + batch_idx * embedding_vector_dim;

    for(unsigned int i = 0; i < sparse_input_dim; ++i){
        unsigned long idx = d_sparse_ptr[batch_idx * sparse_input_dim + i] * embedding_vector_dim;
        if(i == 0){
            ctrl->loadArrayFromCache<float>(0, idx, line, embedding_vector_dim);
        }else{
            ctrl->accumulateArrayFromCache<float>(0, idx, line, embedding_vector_dim);
        }
    }

    // get average
    for(unsigned int i = 0; i < embedding_vector_dim; ++i){
        line[i] /= sparse_input_dim;
    }
}
    ...
    unsigned int batch_size, unsigned int sparse_input_dim, unsigned int embedding_vector_dim)
{
    ...
    unsigned int total_threads = batch_size;
    unsigned int total_blocks = total_threads / threads_per_block + (total_threads % threads_per_block == 0 ? 0 : 1);
    dim3 block(threads_per_block);
    dim3 grid(total_blocks);
