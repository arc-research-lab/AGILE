...
// __global__ void rand_write_kernel(AGILE_CTRL * ctrl, unsigned long * offsets, unsigned int dev_num, unsigned int dev_off, unsigned int req_pre_dev){
//     unsigned int tid = blockIdx.x * blockDim.x + threadIdx.x;
//     if(tid >= req_pre_dev * dev_num){
//         return;
//     }

//     unsigned int dev_id = tid / req_pre_dev;
...
//     // printf("req_num: %d, tid: %d, dev_num: %d, dev_range: %d, dev_id: %d, dev_off: %d, blk_start_idx: %lu\n", req_num, tid, dev_num, dev_range, dev_id, dev_off, offsets[tid]);
//     // agileArr[dev_id + dev_off][blk_start_idx];
//     ctrl->writeThroughNvme_noRead_benchWrite<unsigned int>(dev_id + dev_off, blk_start_idx, tid, chain);
//     atomicAdd(&(logger->write_threads), 1);
//     if(tid == 0){
//         while((atomicAdd(&(logger->write_threads), 0) < req_pre_dev * dev_num) || (atomicAdd(&(logger->finished_write), 0) < atomicAdd(&(logger->issued_write), 0))){
//             // printf("wait: %d thread: %d finish %d issue: %d\n", logger->write_threads, req_pre_dev * dev_num, atomicAdd(&(logger->finished_write), 0), atomicAdd(&(logger->issued_write), 0));
//         }
//         // printf("write: %d\n", logger->write_threads);
//         // printf("wait: %d thread: %d finish %d issue: %d\n", logger->write_threads, req_pre_dev * dev_num, atomicAdd(&(logger->finished_write), 0), atomicAdd(&(logger->issued_write), 0));
//     }
// }
...
__global__ void rand_write_kernel(AGILE_CTRL * ctrl, unsigned long * offsets, unsigned int dev_num, unsigned int dev_off, unsigned int req_pre_dev)
{
    ...
    for(unsigned int i = dev_off; i < dev_num + dev_off; ++i){
        ...
        ctrl->writeThroughNvme_noRead_benchWrite<unsigned int>(i, blk_start_idx, tid, chain);
    }
    atomicAdd(&(logger->write_threads), 1);
    if(tid == 0){
        while((atomicAdd(&(logger->write_threads), 0) < req_pre_dev) || (atomicAdd(&(logger->finished_write), 0) < atomicAdd(&(logger->issued_write), 0))){
            // printf("wait: %d thread: %d finish %d issue: %d\n", logger->write_threads, req_pre_dev * dev_num, atomicAdd(&(logger->finished_write), 0), atomicAdd(&(logger->issued_write), 0));
        }
    }
}
...
int main(int argc, char ** argv){
    ...
    host.addNvmeDev("0x97000000", 32768, 0, 32, cfg.queue_depth); // b1:00.0  1
    ...
    rand_write_kernel<<<thread_blocks, cfg.thread_dim, 0, cuda_stream>>>(ctrl, d_random_offsets, cfg.ssd_num, cfg.ssd_off, cfg.req_pre_dev);
    ...
    unsigned int total_cmd = host.h_logger->issued_write;
